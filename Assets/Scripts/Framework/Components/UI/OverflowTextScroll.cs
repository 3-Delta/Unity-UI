...
public class OverflowTextScroll : MonoBehaviour {
    ...
    private void Update() {
        if (this.canMove) {
            ...
            curAnchorPos = this.selfRect.localPosition;
            ...
            this.selfRect.localPosition = this.curAnchorPos;
        }
    }
    ...
    public void Begin() {
        this.StopCoroutine(nameof(Start));
        this.StartCoroutine(nameof(Start));
    }
    ...
    private bool Judge() {
        ...
        */
        ...
        // 方案2：用localPosition的方式计算
        this.leftEdge = this.parentRect.localPosition.x - (displayWidth / 2f) - (1 - this.selfRect.pivot.x) * textWidth;
