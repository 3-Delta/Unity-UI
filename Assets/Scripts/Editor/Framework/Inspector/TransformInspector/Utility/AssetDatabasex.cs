...
public static class AssetDatabasex {
    ...
    public static T LoadAssetOfType<T>(string contains = null, SearchFilter searchAssets = SearchFilter.All,
        Action error = null, Action success = null) where T : UO {
        ...
        foreach (var assetGUID in assetGUIDs) {
            ...
            if (string.IsNullOrEmpty(assetPath) || !allowScriptAssets && assetPath.EndsWith(".cs") ||
                contains != null && !Path.GetFileName(assetPath).Contains(contains))
                continue;
