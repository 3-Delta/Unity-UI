...
namespace Ludiq.PeekCore
{
	...
	{
		public AdditionHandler() : base(OperatorCategory.Math, BinaryOperator.Addition, "Addition", "Add", "+", "+", "op_Addition")
		{
			...
			Handle<string, string>((a, b) => a + b, typeof(string));
		}
		...
		public override string GetDescriptionFormat(Type leftType, Type rightType)
		{
			...
			{
				return "Returns the concatenation of {0}.";
			}
			else if (leftType.IsPrimitive && rightType.IsPrimitive)
			{
				return "Returns the sum of {0}.";
			}
			else
			{
				return "Returns the result of adding {0}.";
			}
		}
