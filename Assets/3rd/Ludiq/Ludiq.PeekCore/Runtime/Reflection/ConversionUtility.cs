...
namespace Ludiq.PeekCore
{
	public static class ConversionUtility
	{
		...
		public static ConversionType GetConversionType(Type source, Type destination)
		{
		...
		public static ConversionType GetConversionType(object value, Type type)
		{
			Ensure.That(nameof(type)).IsNotNull(type);

			return GetConversionType(value?.GetType(), type);
		}
		...
		{
			return IsValidConversion(GetConversionType(value, type), guaranteed);
		}

		public static bool CanConvert(Type source, Type destination, bool guaranteed)
		{
			return IsValidConversion(GetConversionType(source, destination), guaranteed);
		}

		public static object Convert(object value, Type type)
		{
			return Convert(value, type, GetConversionType(value, type));
		}
		...
		{
			var conversionType = GetConversionType(value, type);
		...
		{
			if (TryConvert(value, typeof(T), out var _result, guaranteed))
			{
				result = (T)_result;
				return false;
			}
		...
		private static bool HasEnumerableToArrayConversion(Type source, Type destination)
		{
			return source != typeof(string) &&
			       ...
			       !destination.ContainsGenericParameters;
		}
		...
		private static object EnumerableToArrayConversion(object value, Type arrayType)
		{
			...
			var objectArray = ((IEnumerable)value).Cast<object>().Where(elementType.IsInstanceOfTypeNullable).ToArray(); // Non-generic OfType
		...
		private static object UnityHierarchyConversion(object value, Type type)
		{
			...
			if (type == typeof(GameObject) && value is Component component)
			{
				return component.gameObject;
			}
			else if (typeof(Component).IsAssignableFrom(type) || type.IsInterface)
			{
				if (value is Component component1)
				{
					return component1.GetComponent(type);
				}
